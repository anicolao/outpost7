# Single Player Mode

Verify starting a solo game and AI Opponent response

## Add 1 Player and Click Start Solo

![Add 1 Player and Click Start Solo](screenshots/000-001-start-solo.png)

**Specs:**
- Start Game with 1 Player
- Verify AI Player Added

## Human (Red) plays a move

![Human (Red) plays a move](screenshots/001-002-human-move.png)

**Specs:**
- Play Red Card

## Wait for AI (Yellow) to play

![Wait for AI (Yellow) to play](screenshots/002-003-verify-ai-response.png)

**Specs:**
- Wait for Turn Change or Move

